...
articleView.handleAuthorFilter = function() {
  ...
  $('#author-filter').on('change', function() {
    ...
    if ( $(this).val()) {
      ...
      console.log(`article[data-author='${$authorChoice}']`);
      $('article').hide();
      $(`article[data-author='${$authorChoice}']`).fadeIn(750);

      // TODO: If the <select> menu was changed to an option that has a value, we first need to hide all the 
      //articles, and then show just the ones that match for the author that was selected.
      // Use an "attribute selector" to find those articles, and fade them in for the reader.

    } else {
      $('article').show();
      $('.template').hide();
      // TODO: If the <select> menu was changed to an option that is blank, we should first show all the 
      //articles, except the one article we are using as a template.
